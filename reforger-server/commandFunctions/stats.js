...
module.exports = async (interaction, serverInstance, discordClient, extraData = {}) => {
    ...
    try {
        ...
        if (rows.length === 0) {
            const serverMessage = filterServerName !== 'all' ? ` on server ${filterServerName}` : '';
        ...
        const hasModHistory = (
            (stats.lightban_streak && stats.lightban_streak > 0) ||
            (stats.heavyban_streak && stats.heavyban_streak > 0) ||
            (stats.kick_streak > 0)
        );
        ...
        if (hasModHistory) {
            ...
            if (stats.lightban_streak && stats.lightban_streak > 0) {
                modHistoryItems.push(`Light Bans: ${stats.lightban_streak}`);
            }

            if (stats.heavyban_streak && stats.heavyban_streak > 0) {
