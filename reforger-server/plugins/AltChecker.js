...
class AltChecker {
  ...
  async prepareToMount(serverInstance, discordClient) {
    ...
    try {
      ...
      this.serverInstance.on("playerJoined", (player) => this.handlePlayerJoined(player));
  ...
  async handlePlayerJoined(player) {
    ...
    try {
      ...
      const altAccounts = rows.filter((dbPlayer) => dbPlayer.playerName !== playerName && dbPlayer.beGUID !== beGUID);
